#ifndef NEOSYSTEM_POPPO_AUTH_GATEWAY_SLACK_LOGIN_
#define NEOSYSTEM_POPPO_AUTH_GATEWAY_SLACK_LOGIN_
	...
	void get_user_info(const callback_func_type& func, const std::string& access_token) {
		auto client = http::https_client::create(logger_, io_context_, cache_, [this, func](
			...
			try {
				...
				bool ok = pt.get("ok", false);
				if (ok == false) {
					log::info(logger_)() << S_ << "error";
					func(503, "");
					return;
				}

				std::string id = pt.get("user.id", "");
		...
		util::urlencode(access_token, encoded_access_token);
		stream << "GET " << url.get_path() << "?token=" << encoded_access_token << " HTTP/1.1\r\n";
...
public:
	slack_login(log::logger& logger, const oauth2_server_config::ptr_type& conf,
	...
	void login(const callback_func_type& func, const std::string& code) {
		auto client = http::https_client::create(logger_, io_context_, cache_, [this, func](
			...
			try {
				...
				//log::info(logger_)() << S_ << "access_token: " << access_token;
